      ...
      <div style={{ background: s.bg2, border: `1px solid ${s.border2}`, borderRadius: 4, width: '100%', maxWidth: 700, maxHeight: '92vh', overflowY: 'auto' }} className="fade-in">
        ...
        <div style={{ padding: 22 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
            {F('Date', 'date', 'date')} {F('Time', 'time', 'time')}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
            {F('Entry $', 'entry', 'number')} {F('Stop $', 'stop', 'number')} {F('Exit $', 'exit', 'number')} {F('Shares / Qty', 'shares', 'number')}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
            {F('P&L ($)', 'pnl', 'number')} {F('R:R Achieved', 'rr')}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '14px 22px', borderTop: `1px solid ${s.border}` }}>
          ...
          <Btn onClick={() => { if (!f.outcome) return; onSave({ ...f, id: f.id || uid() }) }} disabled={saving}>{saving ? 'Saving...' : 'Save Trade'}</Btn>
        </div>
      </div>
    </div>
  )
